...
struct ContentView: View {
    ...
    @State private var moveBackgrounImage = false
        ...
        GeometryReader { geo in
            ZStack {
                Image("hogwarts")
                    ...
                    .padding(.top, 3)
                    .offset(x: moveBackgrounImage ? geo.size.width/1.1 : -geo.size.width/1.1)
                    .onAppear {
                        withAnimation(.linear(duration: 60).repeatForever()){
                            moveBackgrounImage.toggle()
                        }
                    }
                    ...
                    VStack {
                        if animateViewsIn {
                            if store.books.contains(.active) == false {
                                Text("No questions available. Go to setting.")
